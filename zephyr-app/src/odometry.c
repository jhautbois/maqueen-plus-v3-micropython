...
int odometry_update(struct odometry *odom)
{
	...
	/* Convert to millidegrees: rad * 180000 / pi ≈ rad * 57296 */
...
void odometry_update_pid_distance(struct odometry *odom, int32_t distance_mm)
{
	...
	odom->dist_left += distance_mm;
	odom->dist_right += distance_mm;

	/* Update position based on current heading */
	/* PID moves in straight line, so no heading change */
	odom->x += (distance_mm * cos_mdeg(odom->heading_mdeg)) / 1000;
	odom->y += (distance_mm * sin_mdeg(odom->heading_mdeg)) / 1000;

	/* Update timestamp */
	odom->last_update_ms = k_uptime_get();

	LOG_DBG("PID distance: %d mm -> pos (%d, %d)", distance_mm, odom->x, odom->y);
}

void odometry_update_pid_rotation(struct odometry *odom, int32_t angle_mdeg)
{
	/* Update heading */
	odom->heading_mdeg += angle_mdeg;
	...
	LOG_DBG("PID rotation: %d mdeg -> heading %d mdeg",
		angle_mdeg, odom->heading_mdeg);
}
